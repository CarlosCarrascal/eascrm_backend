...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initAdminEnhancements() {
    ...
    // Corregir espacios en container-fluid
    fixContainerFluid();
  }
  
  // Función para corregir los espacios en container-fluid
  function fixContainerFluid() {
    const containers = document.querySelectorAll('.container-fluid');
    containers.forEach(container => {
      container.style.padding = '0';
      container.style.margin = '0';
      
      // Si el contenedor está causando espacio extra, ajustar su altura
      if (container.offsetHeight > container.scrollHeight) {
        container.style.height = 'auto';
      }
    });
    
    // Corregir espacio en las vistas de lista y detalle
    if (document.body.classList.contains('app-CRMapp')) {
  ...
  // Llamar a la función después de una breve espera para asegurar que todos los elementos estén cargados
  setTimeout(fixContainerFluid, 100);
});
